...
describe('Send Forgot Password M', () => {
  ...
  it('should not be able to send a forgot password email if user not exists', async () => {
    await expect(sendForgotPasswordMailUseCase.execute('test_email2@test.com')).rejects.toEqual(
      new AppError('User does not exists'),
    );
  });

  it('should be able to create an users token', async () => {
    const generatedTpkenEmail = jest.spyOn(usersTokenRepositoryinMemory, 'create');
